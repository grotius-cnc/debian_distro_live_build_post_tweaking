...
cd iso
# mksquashfs zips (compresses) the chroot directory into a file "filesystem.squashfs" with -comp xz (compression type). 
# "-e boot"  packing means without "/boot" directory. That is no good.
sudo mksquashfs chroot $DIR/iso/binary/live/filesystem.squashfs -comp xz 
...
sudo xorriso \
	...
	-o live-image-amd64.hybrid-modified.iso binary
